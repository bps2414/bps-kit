...
### Architecture
- **Active skills**: in `.copilot-skills/` — see `ARCHITECTURE.md` for full list
...
| 2 | Read agent's `.md` file? | → STOP. Open `.github/agents/{agent}.md` |
...
- Skill-level scripts: `.copilot-skills/<skill>/scripts/`
...
> 🔴 **Agents & Skills can invoke ANY script** via `python .copilot-skills/<skill>/scripts/<script>.py`

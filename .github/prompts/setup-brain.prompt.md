...
3. **Check de Ativas vs Inativas**:
   - Compare o que o repositório usa com as skills que estão na pasta local de configuração (`.copilot-skills/`).
...
5. **Execução Automática da Otimização**:
   - Assim que o usuário aprovar, utilize tools de shell (como `run_command` via bash ou node) ou scripts nativos para MOVER as pastas de skills sugeridas de `.copilot-vault/` para `.copilot-skills/`.
   - Modifique regras do `.github/copilot-instructions.md` se for preciso rebalancear ou limpar referências.
